...
public class GetGroupOverlapQueryHandlerTests
{
    ...
    public async Task Handle_RanksWindowsByAvailableCountDescending()
    {
        ...
        instances.Instances.Add(new AvailabilityInstance { UserId = userA, Date = Date(0), StartTime = new TimeOnly(18, 0), Status = AvailabilityStatus.Available });
        // Tuesday evening: userA and userB free.
        instances.Instances.Add(new AvailabilityInstance { UserId = userA, Date = Date(1), StartTime = new TimeOnly(18, 0), Status = AvailabilityStatus.Available });
        instances.Instances.Add(new AvailabilityInstance { UserId = userB, Date = Date(1), StartTime = new TimeOnly(18, 0), Status = AvailabilityStatus.Available });

        var handler = new GetGroupOverlapQueryHandler(members, instances, new FakeCurrentUserService(userA));

        var result = await handler.Handle(
            new GetGroupOverlapQuery(groupId, Date(0), Date(1), WeekendOnly: false),
    ...
    public async Task Handle_WeekendOnlyFilter_ExcludesWeekdays()
    {
        ...
        var instances = new FakeAvailabilityInstanceRepository();
        instances.Instances.Add(new AvailabilityInstance { UserId = userA, Date = Date(0), StartTime = new TimeOnly(18, 0), Status = AvailabilityStatus.Available }); // Monday
        instances.Instances.Add(new AvailabilityInstance { UserId = userA, Date = Date(5), StartTime = new TimeOnly(18, 0), Status = AvailabilityStatus.Available }); // Saturday

        var handler = new GetGroupOverlapQueryHandler(members, instances, new FakeCurrentUserService(userA));

        var result = await handler.Handle(
            new GetGroupOverlapQuery(groupId, Date(0), Date(6), WeekendOnly: true),
            CancellationToken.None);
    ...
    public async Task Handle_NonMember_ThrowsNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => handler.Handle(
            new GetGroupOverlapQuery(groupId, Date(0), Date(1), false), CancellationToken.None));
    }
}
